...
export default function ProductScrollingImages (props: Props) {
	...
	const imageElems = images.map((img, i) => {
		return (
			<div key={i} className='aspect-square w-full absolute z-[-1]' style={{backgroundImage: `url('${img}')`, backgroundSize: '100%', backgroundPositionY: '-40px' }} />
		)
	});

	return (
		<div className='relative aspect-square w-full overflow-hidden rounded-md bg-no-repeat flex justify-center items-end flex-col z-0' >
				<div className="aspect-square w-full absolute z-[-1]">
					{imageElems[selectedImage]}
				</div>
				<div className="aspect-square w-full absolute z-[-1] bottom-[-110%]">
			...
			<div className="flex flex-row h-min w-full justify-center absolute bottom-4">
				{ images.map((img, i) => {
					return (
